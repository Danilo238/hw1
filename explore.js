...
function onJsonImage(response)
{
    ...
    for(let i = 0; i < responseGame.length; i++)
    {
        if(responseGame[i].cover === response[0].id)
        {
            ...
            const k = container.dataset.index;
...
function search(event)
{
    ...
    fetch("games_by_platform.php?platform=" + encodeURIComponent(selected) + "&token=" + encodeURIComponent(token)).then(onResponse).then(onJsonGames);
}
